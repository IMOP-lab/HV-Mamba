...
class SpatialTemporalDynamicAttention(nn.Module):
...
class FFTVSSBlock(nn.Module):
    def __init__(self, channel):
        super(FFTVSSBlock, self).__init__()
...
class HybridCoordFreqAttention(nn.Module):
    def __init__(self, in_channels, reduction=32, num_iterations=3):
        super(HybridCoordFreqAttention, self).__init__()
...
class SimpleImplicitFeaturizer(torch.nn.Module):
    ...
    def forward(self, original_image):
        ...
        # Create a grid of coordinates normalized between -1 and 1
        ...
        # Broadcast the coordinates to match the batch size
        ...
        # Reshape and create the final feature set
...
class IFA(torch.nn.Module):
    ...
    def forward(self, source, guidance):
        ...
        # Upsample the source image
        up_source = F.interpolate(source, (h * 2, w * 2), mode="bilinear", align_corners=True)
        # Generate low-resolution and high-resolution coordinates
        ...
        # Interpolate low-resolution coordinates to match high-resolution size
        up_lr_coords = F.interpolate(lr_coords, (h * 2, w * 2), mode="bilinear", align_corners=True)
        coord_diff = up_lr_coords - hr_coords

        # Generate features based on coordinate differences
        coord_diff_feats = self.sin_feats(coord_diff)
        c2 = coord_diff_feats.shape[1]
        
        # Broadcast the coordinate features to match the batch size
        bcast_coord_feats = coord_diff_feats.expand(b, -1, -1, -1)
        
        # Combine the upsampled source and coordinate features and process with MLP
...
# class SpatialFeatureEnhancementWithAttention(nn.Module):
#     def __init__(self, in_channels, out_channels, r=4):
#         super(SpatialFeatureEnhancementWithAttention, self).__init__()
#         inter_channels = int(out_channels // r)

#         # Scene encoding for X and Y directions
#         self.scene_encoder_x = nn.Sequential(
#             nn.Conv2d(in_channels, out_channels, kernel_size=(1, 3), stride=1, padding=(0, 1)),
#             nn.BatchNorm2d(out_channels),
#             nn.ReLU(True)
#         )
        
#         self.scene_encoder_y = nn.Sequential(
#             nn.Conv2d(in_channels, out_channels, kernel_size=(3, 1), stride=1, padding=(1, 0)),
#             nn.BatchNorm2d(out_channels),
#             nn.ReLU(True)
#         )
...
#         self.self_attention = nn.MultiheadAttention(embed_dim=out_channels, num_heads=8)
...
#         scene_feat_x = self.scene_encoder_x(feature)
#         scene_feat_y = self.scene_encoder_y(feature)
#         content_feat = self.content_encoder(feature)
        
#         combined_feat = torch.cat((scene_feat_x + scene_feat_y, content_feat), dim=1)  
...
#         content_feat_flat = content_feat.view(b, c, -1).permute(2, 0, 1)
#         attn_output, _ = self.self_attention(content_feat_flat, content_feat_flat, content_feat_flat)
#         attn_output = attn_output.permute(1, 2, 0).view(b, c, h, w)
        
#         enhanced_feature = content_feat * attention_feat + content_feat * (1 - attention_feat) + attn_output
#         enhanced_feature = enhanced_feature + residual

#         return enhanced_feature
from torch.nn import Module, Conv2d, Parameter, Softmax
class nmcs(nn.Module):
    def __init__(self, in_channels, groups=2):  #ConfluentSpatioChannelModule
        super().__init__()
        self.groups = groups
...
class UNet_mamba(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False):
        ...
        self.bilinear = bilinear
        # self.repvit = repvit_m1_1(pretrained=True, num_classes=0)
        self.inc = DoubleConv(32, 64)
        self.inc1 = DoubleConv(3, 64)
        # self.sfea=SpatialFeatureEnhancementWithAttention(64,64)
        ...
        # self.up1 = IFAUpsampler(1024,bilinear=bilinear)
        # self.up2 = IFAUpsampler(512,bilinear=bilinear)
        # self.up3 = IFAUpsampler(256,bilinear=bilinear)
        # self.up4 = IFAUpsampler(128,bilinear=bilinear)       
        self.up2 = Up(1024, 512 // factor, bilinear)
        self.up3 = Up(512, 256 // factor, bilinear)
        self.up4 = Up(256, 128, bilinear)
        self.outc = OutConv(128, n_classes)
        self.fft_out = FFTVSSBlock(channel=3)
        self.combined_attention = SpatialTemporalDynamicAttention(
            channels=1024,
            num_heads=8,
            kernels=[1, 3, 5, 7],
            reduction=16,
        ...
        self.freq_attn = HybridCoordFreqAttention(64)
        self.freq_attn128 = HybridCoordFreqAttention(128)
        self.freq_attn256 = HybridCoordFreqAttention(256)
        self.freq_attn512 = HybridCoordFreqAttention(512)

        self.freq_attn1024 = HybridCoordFreqAttention(1024)
        ...
        self.nmcs1=nmcs(2048)
        self.nmcs2=nmcs(1024)
        self.nmcs3=nmcs(512)
        self.nmcs4=nmcs(256)
        self.nmcs5=nmcs(128)        
