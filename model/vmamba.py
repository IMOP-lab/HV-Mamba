...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        x = x.reshape(B, self.d_inner, H, W)
        ...
        y = y * F.silu(z)
        out = self.out_proj(y)
        if (dropout := self.dropout) is not None:
            out = dropout(out)
        return out
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        B, C, H, W = input.shape
        ln_output = self.ln_1(input.view(B, C, -1).transpose(1, 2)).transpose(1, 2).view(B, C, H, W)

        attn_output = self.self_attention(ln_output)
